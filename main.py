...
# 📌 ChatGPT 모델 선택
chat_model = "gpt-3.5-turbo"  # 또는 "gpt-4"
...
def fetch_chatgpt_answer(prompt):
    response = openai.ChatCompletion.create(
        model=chat_model,
        messages=[
            {"role": "system", "content": "너는 유익하고 친절한 AI 조수야."},
...
def process_question(request: QueryRequest):
    ...
    prompt = generate_prompt(results, user_question)
    logger.info(f"\n🚀 [DEBUG] ChatGPT에 전달될 프롬프트:\n{prompt}\n{'='*80}")

    try:
        answer = fetch_chatgpt_answer(prompt)
        logger.info(f"\n✅ [DEBUG] 최종 답변:\n{answer}\n{'='*80}")
    except Exception as e:
        logger.error(f"ChatGPT 요청 중 오류 발생: {str(e)}")
        return {"error": f"ChatGPT 요청 중 오류 발생: {str(e)}"}
